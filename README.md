...
1. Create the Neon project and the Qdrant Cloud cluster; note their connection details.
...
3. Set the secrets `render.yaml` leaves blank (`sync: false`): `ANTHROPIC_API_KEY`,
   `POSTGRES_HOST`/`PORT`/`DB`/`USER`/`PASSWORD` (from Neon), `QDRANT_URL`/`QDRANT_API_KEY` (from
   Qdrant Cloud), and optionally `SEED_ADMIN_PASSWORD`/`SEED_USER_PASSWORD`. `SESSION_SECRET` is
